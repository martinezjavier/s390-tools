	       ...
	       int* index, int boot_mounted)
{
	...
	if (strncmp("linux", &file->buffer[key_start], key_end - key_start) == 0) {
		misc_asprintf(&val, "%s%s", boot_mounted ? "/boot" : "",
...
scan_bls(const char* blsdir, struct scan_token** token, int scan_size)
{
	...
	mounted = util_proc_is_mountpoint("/boot");
	if (mounted < 0) {
		error_reason("Error checking if /boot is a mount point");
		return rc;
	}
	...
	while (n--) {
		...
		while ((size_t)file.pos < file.length) {
			...
			case '\0':
				...
				break;
			default:
				scan_bls_field(&file, buffer, &count, mounted);
				break;
			}
		}
